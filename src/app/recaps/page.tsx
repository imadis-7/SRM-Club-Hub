    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-4xl font-headline font-bold">Smart Recap Archive</h1>
          <p className="text-muted-foreground font-medium">Search, browse, and generate AI-powered summaries of past sessions.</p>
        </div>
        <div className="w-full md:w-96 relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Search past meetings..." className="pl-10 rounded-xl bg-white shadow-sm border-border/50" />
        </div>
      </header>

      <section className="bg-gradient-to-r from-primary/10 to-indigo-500/10 p-8 rounded-3xl border border-primary/20 relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <h2 className="text-3xl font-headline font-bold">Instant Auto-Recap</h2>
            <p className="text-muted-foreground text-lg leading-relaxed">
              Paste your meeting transcripts or raw notes below. SRM AI will synthesize key discussion points and extract action items instantly.
            </p>
            <div className="space-y-4">
              <Textarea 
                placeholder="Paste transcript or meeting logs here..." 
                className="min-h-[150px] bg-white rounded-2xl p-4 shadow-sm border-none focus-visible:ring-2 focus-visible:ring-primary/20"
                value={meetingNotes}
      ...
      <section className="space-y-6">
        <h3 className="text-2xl font-headline font-bold">Recently Archived</h3>
            ...
            <Card key={recap.id} className="group hover:shadow-xl transition-all border-none bg-white shadow-lg shadow-indigo-500/5">
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="w-3 h-3" /> {recap.date}</span>
                </div>
                <CardTitle className="font-headline text-xl group-hover:text-primary transition-colors">{recap.title}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed">{recap.summary}</p>
                <div className="flex items-center justify-between pt-4 border-t border-border/50">
                  ...
                  <Button variant="ghost" size="sm" className="text-primary hover:text-primary hover:bg-primary/10 font-bold rounded-lg text-xs">
      ...
      <Dialog open={showRecapDialog} onOpenChange={setShowRecapDialog}>
        <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col p-0 overflow-hidden rounded-3xl">
          <DialogHeader className="p-8 bg-gradient-to-r from-primary to-indigo-600 text-white shrink-0">
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className="w-5 h-5" />
              <span className="text-xs font-bold uppercase tracking-widest opacity-80">AI-Generated Meeting Recap</span>
            </div>
            <DialogTitle className="text-3xl font-headline font-bold">Summary & Insights</DialogTitle>
          </DialogHeader>
            ...
            {currentRecap && (
              <div className="space-y-8 pb-4">
                <section className="space-y-3">
                  ...
                  <p className="text-foreground leading-relaxed font-medium bg-muted/30 p-4 rounded-xl border border-border/50">
                    {currentRecap.summary}
                  </p>
                </section>

                <section className="space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-primary">Discussion Points</h4>
                  <ul className="space-y-2">
                    {currentRecap.discussionPoints.map((point, i) => (
                      <li key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white border shadow-sm text-sm">
                ...
                <section className="space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-primary">Action Items</h4>
                    ...
                    {currentRecap.actionItems.map((item, i) => (
                      <div key={i} className="p-4 rounded-xl border border-emerald-100 bg-emerald-50/30 space-y-2 shadow-sm">
                        <div className="flex justify-between items-start gap-4">
                          <p className="text-sm font-bold text-emerald-900">{item.description}</p>
                          {item.dueDate && <Badge className="bg-emerald-200 text-emerald-800 border-none text-[9px]">{item.dueDate}</Badge>}
                        </div>
                        {item.assignee && (
                          <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-600 uppercase tracking-tight">
          ...
          <DialogFooter className="p-6 border-t bg-muted/20 shrink-0">
            <Button variant="outline" className="rounded-xl border-border/50 font-bold" onClick={() => setShowRecapDialog(false)}>Close</Button>
            <div className="flex gap-2">
              ...
              <Button className="rounded-xl px-6 font-bold shadow-md">Add to Archive</Button>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>
